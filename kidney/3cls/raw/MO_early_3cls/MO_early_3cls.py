...
omics_list = ["mRNA", "miRNA", "methy"]
mRNApath = "./kidney_3cls_mRNA_rf.csv"
miRNApath = "./kidney_3cls_miRNA_rf.csv"
methypath = "./kidney_3cls_methy_rf.csv"
files_list = [mRNApath, miRNApath, methypath]

# read data
data_mRNA = pd.read_csv(mRNApath, index_col=0)
data_miRNA = pd.read_csv(miRNApath, index_col=0)
data_methy = pd.read_csv(methypath, index_col=0)

# check data
print('_' * 50)
print('mRNA sample counts：', data_mRNA.shape[0],
      '\nmRNA feature counts：', data_mRNA.shape[1] - 1)
print('_' * 50)
print('miRNA sample counts：', data_miRNA.shape[0],
      '\nmiRNA feature counts：', data_miRNA.shape[1] - 1)
print('_' * 50)
print('methy sample counts：', data_methy.shape[0],
      '\nmethy feature counts：', data_methy.shape[1] - 1)

# train data X
X = pd.concat([data_mRNA.iloc[:, :-1], data_miRNA.iloc[:, :-1], data_methy.iloc[:, :-1]], axis=1).values
# label Y
Y = data_mRNA.label.values
...
for i in ["MOCCT", "BP", "DT", "SVM", "NB"]:
    omicsfolder = os.path.join(savepath, i)
...
for fold, (train_idx, test_idx) in enumerate(skf.split(X, Y)):
    ...
    y_pred_train = torch.argmax(mocct(x_train, x_train, x_train), dim=1)
    y_pred_test = torch.argmax(mocct(x_test, x_test, x_test), dim=1)
    ...
    loss_fn_bp = nn.CrossEntropyLoss()
    epochs = 2000
    ...
    y_pred_train_bp = torch.argmax(bp(x_train), dim=1)
    y_pred_test_bp = torch.argmax(bp(x_test), dim=1)
...
metrics_table.to_csv("MO_early_3cls_metrics_result.csv")
